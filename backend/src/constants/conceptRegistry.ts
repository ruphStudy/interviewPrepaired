/**
 * Canonical technical-concept registry used by AnswerSignalService (2B) to
 * scan answer text deterministically (no AI call). This is intentionally a
 * DUPLICATE of frontend/src/utils/conceptRegistry.ts — the frontend uses its
 * own copy for local, offline, while-speaking concept detection (2C), and
 * this is a separate TypeScript project with no shared package between
 * backend and frontend.
 *
 * KEEP THE CANONICAL KEY SET (and aliases) IN SYNC WITH
 * frontend/src/utils/conceptRegistry.ts. The matching logic (detectConcepts)
 * is also duplicated on purpose — do not import across the frontend/backend
 * boundary.
 */
